This theme or plugin is comprised of two parts.

(1) the PHP code and integrated HTML are licensed under the GNU AFFERO GENERAL PUBLIC LICENSE (AGPL).
You will find a copy of the AGPL in the same directory as this text file.

(2) All other parts, but not limited to the CSS code, images, and design are
licensed according to the license purchased from Envanto.

Read more about licensing here: http://codecanyon.net/licenses
