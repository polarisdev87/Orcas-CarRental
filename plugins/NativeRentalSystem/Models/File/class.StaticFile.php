<?php
/**
 * NRS File operations Static methods
 ...
 */
namespace NativeRentalSystem\Models\File;
...
class StaticFile
{
     ...
     * @param $paramPath
     * @param $paramFileExtension
     * @return array
     */
    public static function getFolderFileList($paramPath, $paramFileExtension)
    {
        $retFiles = array();
        $validPath = sanitize_text_field($paramPath);
        $validFileExtension = sanitize_text_field($paramFileExtension);
        $dirFiles = array_diff(scandir($validPath), array('..', '.'));

        foreach($dirFiles AS $dirFile)
        {
            $extension = strtolower(pathinfo($validPath.$dirFile, PATHINFO_EXTENSION));
            // Case-insensitive check
            if($extension === $validFileExtension)
            {
                $retFiles[] = $dirFile;
            }
        }
