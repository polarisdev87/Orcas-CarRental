...
class ExtraDiscount extends AbstractElement implements iElement
{
    protected $conf 	                = NULL;
    protected $lang 		            = NULL;
    ...
    public function getExtraId()
    {
        ...
        {
            $retExtraId = $discountData['extra_id'];
        }
        return $retExtraId;
    }
    ...
    public function save()
    {
        ...
        $validExtraId = isset($_POST['extra_id']) ? StaticValidator::getValidPositiveInteger($_POST['extra_id'], 0) : 0;
        $validDiscountType = isset($_POST['discount_type']) && $_POST['discount_type'] == 4 ? 4 : 3;
        ...
        $periodCheckQuery = "
            SELECT discount_id
            FROM {$this->conf->getPrefix()}discounts
            WHERE discount_id!='{$validDiscountId}' AND discount_type='{$validDiscountType}' AND extra_id='{$validExtraId}' AND (
        ...
        {
            $ok = FALSE;
            $this->errorMessages[] = $this->lang->getText('NRS_EXTRA_DISCOUNT_DAYS_INTERSECTION_ERROR_TEXT');
        }
        ...
        if($validDiscountId > 0 && $ok)
        {
            $updateSQL = "
                ...
                extra_id='{$validExtraId}', period_from='{$validPeriodFrom}', period_till='{$validPeriodTill}',
            ...
            if($saved === FALSE)
            {
                $this->errorMessages[] = $this->lang->getText('NRS_EXTRA_DISCOUNT_UPDATE_ERROR_TEXT');
            } else
            {
                $this->okayMessages[] = $this->lang->getText('NRS_EXTRA_DISCOUNT_UPDATED_TEXT');
            }
        } else if($ok)
        {
            $insertSQL = "
                ...
                VALUES
                (
                    '{$validDiscountType}', '0', '0', '{$validExtraId}',
            ...
            {
                $this->errorMessages[] = $this->lang->getText('NRS_EXTRA_DISCOUNT_INSERT_ERROR_TEXT');
            } else
            {
                $this->okayMessages[] = $this->lang->getText('NRS_EXTRA_DISCOUNT_INSERTED_TEXT');
            }
        }
    ...
    public function delete()
    {
        ...
        {
            $this->errorMessages[] = $this->lang->getText('NRS_EXTRA_DISCOUNT_DELETE_ERROR_TEXT');
        } else
        {
            $this->okayMessages[] = $this->lang->getText('NRS_EXTRA_DISCOUNT_DELETED_TEXT');
        }
